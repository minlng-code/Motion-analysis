...
def log_session(
    ...
    rom_score (%), fatigue_flag ('Low'/'Moderate'/'High') được thêm vào assessment.
    """
    init_db()
    ...
    if reps >= 10:
        base_assess = "Excellent"
    elif reps >= 5:
        base_assess = "Good"
    else:
        base_assess = "Keep Trying"

    extra = []
    if rom_score is not None:
        extra.append(f"ROM {rom_score:.1f}%")
    if fatigue_flag is not None:
        extra.append(f"Fatigue {fatigue_flag}")

    if extra:
        assessment = base_assess + " | " + " | ".join(extra)
    else:
    ...
    try:
        ...
        c.execute(
            ...
            (timestamp, patient_name, exercise, reps, min_angle, max_angle, assessment)
            VALUES (?, ?, ?, ?, ?, ?, ?)
        """,
            (
                timestamp,
        ...
        print(f"Data saved: {reps} reps, {assessment}")
        return True
